 ...
 *  STATIC DEFINES
 **********************/
#define THREAD_ID_SELF                           ((THREAD_ID_E)CONFIG_THREAD_ID_TEST1)
#define THREAD_NAME                              "ttest1"

/** Max payload per mempool block (bytes) */
#define THREAD_TEST1_MSG_MAX_PAYLOAD       64U
#define THREAD_TEST1_MSG_SLAB_BLOCK_SIZE   (sizeof(thread_msg_t) + THREAD_TEST1_MSG_MAX_PAYLOAD)
#define THREAD_TEST1_MSGQ_DEPTH            16U
#define THREAD_TEST1_MSG_SLAB_NUM          (THREAD_TEST1_MSGQ_DEPTH + 2U)
/** Max payload on rt_malloc path; caps abnormal lengths from exhausting the heap */
#define THREAD_TEST1_MSG_MALLOC_MAX_PAYLOAD (256U)
 ...
 *  STATIC VARIABLES
 **********************/
static struct rt_thread  s_test1_thread;
static rt_uint8_t        s_test1_stack[CONFIG_THREAD_STACK_TEST1];

static struct rt_mailbox s_test1_msgq;
static rt_ubase_t        s_test1_msgq_buf[THREAD_TEST1_MSGQ_DEPTH];

static struct rt_mempool s_test1_msg_slab;
static rt_uint8_t        s_test1_msg_slab_buf[
    THREAD_MP_POOL_SIZE(THREAD_TEST1_MSG_SLAB_BLOCK_SIZE, THREAD_TEST1_MSG_SLAB_NUM)];

static thread_control_t s_thread_ctr_test1 = {
    .f_thread_init          = _thread_init,
    .f_thread_deinit        = _thread_deinit,
    .msgq                   = &s_test1_msgq,
    .msgq_buf               = s_test1_msgq_buf,
    .msgq_depth             = THREAD_TEST1_MSGQ_DEPTH,
    .msg_slab               = &s_test1_msg_slab,
    .msg_slab_buf           = s_test1_msg_slab_buf,
    .msg_slab_block_size    = THREAD_TEST1_MSG_SLAB_BLOCK_SIZE,
    .msg_slab_num           = THREAD_TEST1_MSG_SLAB_NUM,
...
static int _thread_init(void)
{
    rt_thread_init(&s_test1_thread,
                   THREAD_NAME,
                   _thread_entry, RT_NULL,
                   s_test1_stack, sizeof(s_test1_stack),
                   (rt_uint8_t)CONFIG_THREAD_PRIO_TEST1,
                   10U);
    s_thread_ctr_test1.tid = &s_test1_thread;
    rt_thread_startup(&s_test1_thread);
    return 0;
}

static void _thread_deinit(void)
{
    if (s_thread_ctr_test1.tid != RT_NULL) {
        rt_thread_detach(s_thread_ctr_test1.tid);
        s_thread_ctr_test1.tid = RT_NULL;
    }
...
static inline void _thread_system_ready_init(void)
{
    ...
    (void)thread_test1_evt_send(EVENT_TEST1_MSG_HELLO);
}
...
static void _thread_entry(void *parameter)
{
    ...
    s_thread_ctr_test1.tid = rt_thread_self();
    ...
    while (true) {
        ...
        rt_event_recv(&s_thread_ctr_test1.notify,
...
static void _thread_dispatch_events(rt_uint32_t ev)
{
    ...
    if ((ev & SYSTEM_THREAD_NOTIRY_MSG_READY) != 0U) {
        ...
        while (s_thread_ctr_test1.msgq != RT_NULL &&
               rt_mb_recv(s_thread_ctr_test1.msgq, &val, RT_WAITING_NO) == RT_EOK) {
            void *p_msg = (void *)(uintptr_t)val;
            _thread_msg_process(p_msg);
            thread_msg_evt_free((void *)&s_thread_ctr_test1, p_msg);
        }
    }
}
 ...
 *   GLOBAL FUNCTIONS
 **********************/
int32_t thread_test1_evt_msg_send(uint32_t msg_id, uint8_t *pdata, uint32_t length)
{
    if (length > THREAD_TEST1_MSG_MALLOC_MAX_PAYLOAD) {
        LOG_E("thread_test1_evt_msg_send length %u > rt_malloc max %u",
              (unsigned)length, (unsigned)THREAD_TEST1_MSG_MALLOC_MAX_PAYLOAD);
        return -1;
    }
    return thread_evt_msg_send_by_task_id((THREAD_ID_E)CONFIG_THREAD_ID_TEST1,
                                          msg_id, pdata, length);
}

int32_t thread_test1_evt_send(uint32_t msg_id)
{
    return thread_test1_evt_msg_send(msg_id, RT_NULL, 0U);
}

int32_t thread_test1_evt_send_delayed(uint32_t msg_id, uint32_t delay_ms)
{
    return thread_evt_send_delayed_by_task_id((THREAD_ID_E)THREAD_ID_SELF, msg_id, delay_ms);
}

int32_t thread_test1_evt_cancel_delayed(uint32_t msg_id)
{
    return thread_evt_cancel_delayed_by_task_id((THREAD_ID_E)THREAD_ID_SELF, msg_id);
}

THREAD_CTRL_SLOT_ENTRY(((THREAD_ID_E)CONFIG_THREAD_ID_TEST1), &s_thread_ctr_test1);

THREAD_MSG_REGISTRY_ENTRY(((THREAD_ID_E)CONFIG_THREAD_ID_TEST1), ((THREAD_ID_E)CONFIG_THREAD_ID_TEST1),
                          EVENT_TEST1_MSG_HELLO, _thread_msg_hello, true);
THREAD_MSG_REGISTRY_ENTRY(((THREAD_ID_E)CONFIG_THREAD_ID_TEST1), ((THREAD_ID_E)CONFIG_THREAD_ID_TEST),
                          EVENT_TEST_MSG_HELLO, _thread_msg_hello1, true);
THREAD_MSG_REGISTRY_ENTRY(((THREAD_ID_E)CONFIG_THREAD_ID_TEST1), ((THREAD_ID_E)CONFIG_THREAD_ID_TEST2),
